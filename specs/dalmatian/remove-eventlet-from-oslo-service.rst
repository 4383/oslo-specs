...
Eventlet is like the heart of oslo.service.
...
This removing will be like an open heart surgery for oslo.service.

As surgeon, we have to plan the surgery. We have to prepare the patient,
we have to ensure that the environment is sterile, and that anesthesia as well
as convalescence are well planned.

The goal is to identify alternatives to the Eventlet features currently used
by oslo.service.
The goal is to identify how to put in place those alternatives.
The goal is to minimize the uncertainty behind such modernization.

The goal of this document is to design the removal of Eventlet from
oslo.service and to define the different milestones to be able to unplug
Eventlet.
...
The problem is that oslo.service is addict to Eventlet.
Indeed 83 percents of oslo.service's features are based on Eventlet.
...
We must decide on how to transition from the current version of oslo.service
to the future version of oslo.service, a version free from Eventlet.

We must decide if the future version oslo.service will again provide all of
...
For the sake of consistency we have to define a couple of rules of
thumbs. Those will be our magnetic north during this migration:
...
to continue using the Eventlet based implementation, or the new one. Some
oslo.service modules may be abandoned, only those imports would have
...
Oslo.service cannot be transitioned in one time. Like for an heart transplant
we need to place oslo.service under the assistance of an artificial heart
during the time of the operation.

To emulate this artificial heart, we propose to introduce the notion of backend
into oslo.service.
...
- 2025.1: move the current implementation into an eventlet backend (the
  default backend in the config);
- 2025.1: implement the next gen backend;
- 2025.2: deprecate the eventlet backend and make next gen the default;
- 2026.1: remove eventlet implementation and move the next gen implementation
  at the root level, and remove the backend notion.
...
time when they will be eager to use the new implementation::
    ...
    from oslo_service.next_gen import service
...
As said previously this is not an acceptable scenario because it will lead to
many useless back and forth at the import level, without any additional added
value for the users.

The backend notion would help us to manage internally the complexity of this
swapping. Users won't suffer from changing their imports again and again.

Using backend is like swapping the values from 2 variables::
...
You need a third variable to realize your swapping. That third variable is our
backend.
...
and give instruction to users, example with the wsgi sub-module::
    ...
    if _options.backend == "eventlet":
        ...
        raise ImportError("WSGI module not found in the next_gen backend...")

Concerning the modules conserved in the next gen implementation, they will
be rewritten by using new underlying libraries, like cotyledon, futurist,
...
If a customers do not move its oslo.service backend to the next gen backend
in the allotted time, then the T.C should be warned. Then the T.C will
...
The motivation behind the creation of the Oslo was to uniformize the solutions
and to reduce the technical debt.
...
As said previously, the ``periodic_task`` sub-module will become a proxy for
...
To implement the new version of the oslo.service' service sub-module we propose
to use Cotyledon.
...
greenlet. By example, cotyledon, or even futurist, uses threads.
...
It will propose the following choices as valid settings::
    ...
    choices=['eventlet', 'next-gen']

And it will defaulted to ``eventlet``, and users
will move this value to ``next-gen`` when they will have cleaned-up the usage
...
As said previously, using the backend notion, and so this option will act
as an artificial heart for oslo.service, allowing us to swap the internal
implementations.
...
of the ``eventlet`` backend. We do not want to pollute the new next gen
...
#. create a next gen sub-module to host the new implementation 
   and add a new backend config defaulting to the Eventlet sub-module
#. deprecate the eventlet sub-module
#. default the backend config to the next gen implementation
#. remove the Eventlet sub-module and remove the backend config option
#. move the next gen implementation at the root level
...
removals of ``oslo_service.wsgi`` and give tracks to follow (uwsgi, flask,
...).
